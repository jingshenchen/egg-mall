...
class GoodsClassService extends Service {
  /**
   *获取所有分类
   *
   * @param {*} obj
  ...
  async newClass(obj) {
    ...
    const ctx = this.ctx;
    const result = await ctx.model.Goodsclass.create({
      goods_class_name,
      goods_class_url,
    });
    if (result) {
  ...
  async showOne(id) {
    const results = await this.ctx.model.Goodsclass.findById(id);
    return {
      results,
      code: 20000,
    };
...
module.exports = GoodsClassService;
