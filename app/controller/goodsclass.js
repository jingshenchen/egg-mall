...
class GoodsClassController extends Controller {
   ...
   */
  async create(ctx) {
    const result = await ctx.service.goodsclass.newClass(ctx.request.body);
    ctx.body = result;
  }
  async show(ctx) {
    // const isVerify = await ctx.helper.verifyToken(ctx, ctx.params.id);
    // console.log(isVerify);
    const result = await ctx.service.goodsclass.showOne(ctx.params.id);
    // console.log(ctx);
    ctx.body = result;
  }
  async update() {
...
module.exports = GoodsClassController;
